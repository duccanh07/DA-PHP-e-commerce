...
class Slider extends CI_Controller {
	public function __construct(){
		...
		$this->data['com']='slider';
	}

	public function index(){
		$this->session->set_userdata(SESSION_SAVE_LIST_IMAGES, array());
		$this->data['list']=$this->Mslider->slider_all(STATUS_ACTIVE, TYPE_SLIDER);
		$this->data['view']='index';
		$this->data['title']='Quản lý slider - Hệ thống quản lý cơ sở dữ liệu';
	...
	public function insert(){
		...
		if ($this->form_validation->run() == TRUE){
			...
			if(isset($_FILES['images']) && $_FILES['images']['name'][0] != null && $_FILES['images']['name'][0] != ''){
				...
				$config['upload_path'] = 'upload/sliders';
				...
				redirect('admin/slider/insert','refresh');
		    }
		} 
		else{
			$this->data['view']='insert';
			$this->data['title']='Thêm slider - Hệ thống quản lý cơ sở dữ liệu';
			$this->load->view('backend/layout', $this->data);
		}
	}
	...
	public function update($id){
		if ($this->Mslider->slider_check_id($id)) {
			...
			if ($this->form_validation->run() == TRUE){
				...
				$this->data['title']='Cập nhật slider - Hệ thống quản lý cơ sở dữ liệu';
				$this->load->view('backend/layout', $this->data);
			}
		} else {
	...
	public function delete($id){
		...
		$listIdOfImages = json_decode($detailSilder['images']);
		unlink('upload/sliders/'.$listIdOfImages[0]);
		redirect('admin/slider','refresh');
	}


	public function update_images_slider() {
			...
			if ($this->Mslider->slider_check_id($_POST['idSlider']) == true) {
				...
				echo json_encode(
					array(
						...
						'msg' => "Update fail, slider don't exist or deleted"
					)
				);
			}
		} else {
