...
class Phantrang{
	...
	function PagePer($total, $current, $limit, $url='')
	{
		...
		{
			$html.= "<li class = 'hidden-xs'><a href='$url/1.html'>Trang đầu</a></li>";
		...
		if($current <= 3)
		{
			for($i = 1; ($i <= 5) && ($i <= $numPage); $i++)
			{
				if($i == $current)
				{
					$html.= "<li class = 'active' ><a class = 'page' data-page='".$i."' id='".$i."'>".$i."</a></li>";
				}
				else
				{
					$html.= "<li><a href='$url/$i.html' class = 'page' data-page='".$i."' id='".$i."'>$i</a></li>";
				}
			}
		}
		else
		...
		{
			$html.="<li><a href='$url/".($current + 1).".html'>Sau</a></li>";
			$html.="<li class = 'hidden-xs'><a href='$url/$numPage.html'>Trang cuối</a></li>";
		}*/
		return $html;
	}
}
